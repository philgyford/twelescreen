...
Twelescreen runs on Node.js and will display a rotating slideshow of the most recent Tweets from your chosen lists of Twitter accounts. It can handle multiple lists of accounts, each with their own URL. Custom themes (HTML/CSS templates) and settings can be applied to all lists or individual ones.
...
* New themes can be made from two HTML files and CSS.
* Themes can be applied to all, some or individual categories, and the menu.
* Google Web Fonts can be used in categories and the menu.
* A Google Analytics ID can be easily added.
* Customisable introductory texts on the menu.
* Images attached to Tweets using Twitter are displayed.
...
1. Get the Twelescreen code [from GitHub](https://github.com/philgyford/twelescreen/).

2. Copy `config/example_basic.yaml` to `config/production.yaml`.

3. [Create a new Twitter app](https://dev.twitter.com/apps/new). Get the four required authorisation tokens and either add them to `config/production.yaml` or set them as environment variables (see the next section).

4. Run the app with:
   
    $ NDOE_ENV=production node app.js
...
5. If you can view your new Twelescreen, start fiddling with the options, as described below...
...
The category key `_defaults` is a special category; any of its settings are used by *all* other categories unless they have their own options specified. This can be seen in the `config/example_twelescreen.yaml` file, which is the one used on [Twelescreen.com](http://www.twelescreen.com/).
...
There is one more environment variable which has no config file option version, `USE_XHR_POLLING`. This should be set to `true` if running Twelescreen on Heroku, which [has problems with socket.io](https://github.com/joyent/node/wiki/Socket.IO-and-Heroku):
...
Twelescreen is extremely configurable, both on a global level and a per-category level.
...
The menu page is the front page of the site. It only appears if there are two or more categories to appear on it (otherwise the user is redirected to single listable category URL).
...
Additionally, a category with the special key of `_defaults` specifies default configuration options that will be used for all real categories, unless a category specifies its own setting. For any individual category setting the order of precedence is:
...
* `greeting_time`  
  ...
  Default: 2000  
  Examples: 500, 3000  
  The greeting is shown on two occasions. First, when the user initially visit's a category's page. Second, when a new Tweet arrives, a greeting is shown first, in order to catch the user's attention. By default the greeting flashes, although this can be changed using CSS in a custom theme. This setting determines how many milliseconds the greeting is shown for. The greeting flashes the same number of times no matter how long the `greeting_time` option is.
...
* `burn_in_text`  
  ...
  This is a joke and you may never need to use it. You know how public screens often have text burnt into them through overuse? If there is a `burn_in_text` string set then it will be displayed constantly on this category's screen. The supplied themes display it very faintly, mimicking burn in.
...
If you're going to do any development on the code (and I write all this mainly as a reminder to myself) you'll also need NPM, Bower and Grunt.
...
[Bower](https://github.com/bower/bower) is used to manage the front-end third-party JavaScript libraries (although, again, those required for Twelescreen area already included in this repository). It reads the requirements from `bower.json` and installs files in the `bower_components/` directory. Install Bower with:
